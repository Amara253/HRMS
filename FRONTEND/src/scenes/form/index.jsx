...
const Setting = () => {
  ...
  const [selectedImageFileName, setSelectedImageFileName] = useState('');
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axiosInstance.get(`/get_admin/${currentUser._id}`);
                const userData = response.data.admin; // Assuming the API returns user data object with a key 'applicant'
        ...
        try {
            switch (field) {
                case 'username':
                    ...
                    await axiosInstance.put(`/adminName/${currentUser._id}`, { name: username });
                    break;
                case 'email':
                    ...
                    // Save email changes
                    setEditEmail(false);
                    await axiosInstance.put(`/adminEmail/${currentUser._id}`, { email });
                    break;
                case 'password':
                    ...
                    await axiosInstance.put(`/adminPassword/${currentUser._id}`, { password});
                    break;
                default:
                    break;
            }
  ...
  const handleUploadImage=async(event)=>
  {
    ...
    try {
      ...
      await axiosInstance.put(`/addAdminimg/${currentUser._id}`, {
  ...
  const handleUploadCV=async(event)=>
  {
    try {
      ...
      await axiosInstance.put(`/addAdmincv/${currentUser._id}`, {
        profilecv: cvPath,
      });
